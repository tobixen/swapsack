...
* **From** — use as a `swap` *source* (the asset you spend). ◑ = the native swap-from for CACAO/RUNE (a Cosmos `MsgDeposit`, no inbound vault) is implemented + gated + unit-tested, and the underlying `MsgDeposit` build/sign/broadcast is now **mainnet-proven** on Maya (the symmetric LP leg of [docs/live-session-2026-08-16.md](docs/live-session-2026-08-16.md)) — but the swap variant's own memo/destination binding has not itself broadcast, and nothing RUNE has; there is no Maya/THORChain testnet wired up. The same ◑ covers ARB and AVAX (implemented, never broadcast) and DASH and ZEC (vault + OP_RETURN memo, Maya-only, no testnet; ZEC over its bespoke signer)
* **Send** — `send` to an external address (a plain transfer, no swap). ✅ = implemented and tested; ◑ = USDC-ETH rides the *same* ERC-20 send path as USDT-ETH (only the contract/decimals differ) but isn't separately covered by a test; the native CACAO/RUNE Cosmos `MsgSend` is implemented + unit-tested (protobuf byte-exact vs cosmpy, signature verified) but its broadcast is **unproven on mainnet** — there is no Maya/THORChain testnet wired up; and the DASH legacy send shares the BTC build/gate/sign path and is unit-tested (signatures verified) but its broadcast is likewise **unproven on mainnet** (no Dash testnet — an opt-in mainnet self-sweep test exists, see docs/testnet.md); ZEC rides a bespoke v4/ZIP-243 signer (bitcoinlib can't sign Zcash) whose sighash is verified against a real mainnet transaction's signature, with the same unproven-broadcast caveat; ARB and AVAX ride Ethereum's own native/ERC-20 send paths with only the chain id and contracts changed (unit-tested against their own gates, never broadcast)
* **Sweep** — `--amount max` sends the maximum amount. ✅ = works: UTXO and token sweeps end at 0 (a token's gas is paid in the native coin); **native account coins (ETH/AVAX/TRX) intentionally retain a small gas reserve** — the fee is only known at send time, and you *want* some left to move tokens or swap later, so the wallet warns rather than draining you to 0. ◑ = DASH/ZEC sweeps end at 0 like BTC, and the ARB/AVAX ones behave exactly like ETH's, but all of them ride the mainnet-unproven broadcasts above. Blank = not yet (native TRX).
* **Liq**  — `add-liquidity` and `withdraw-liquidity` provide/withdraw liquidity, single-sided by default and *two-sided* with `--symmetric` (EVM assets only, pairing with your own RUNE/CACAO), now including ERC-20 tokens (e.g. USDT-ETH on Maya, via the router). ◑ = DASH/ZEC LP is Maya-only (`--backend maya`, pairs with CACAO) and rides their mainnet-unproven broadcasts; ARB LP is likewise Maya-only and mainnet-unproven. AVAX is blank rather than ◑: its pools exist only on THORChain, which has LP deposits globally paused, so there is no network to add on today — `add-liquidity` refuses up front and names the mimir key. Experimental; see below.

Other features:

* `quote` — read-only price preview for any supported asset
* `status` — track a swap by its inbound txid. For a BTC hash it also prints what the transaction did on-chain (inputs, each output, change, fee in sats and EUR) — so a plain `send`, which no swap vault ever observes, still shows something useful instead of an empty stage list. **Chainflip swaps are tracked too**, keyed by that same deposit txid (a vault swap leaves no channel or order id behind): it reports Chainflip's own state, what went in, what came out and the payout transaction — or, if the price moved and fill-or-kill sent the deposit back rather than filling worse, that it was refunded and for how much, not a payout still pending. That lookup runs before the THORChain/Maya one, because a Chainflip deposit is invisible to those and their honest "not observed" reads as *your money went nowhere*. An amount in an asset this wallet has no key for (SOL, DOT, …) is shown in base units and labelled, rather than scaled by guessed decimals. Chainflip witnesses a deposit only **after it confirms**, so in the window right after broadcasting one — when you are most likely to ask — it answers 404; `status` then decodes the deposit's own OP_RETURN instead and reports which published protocol vault was paid, how much, the destination the payload names and the floor it enforces. That needs no service to be up; a payload paying an address the current vault list does not publish is flagged (vaults rotate every few days, so this is a reason to check, not proof the swap failed). Every view that names a settling protocol opens with a **`backend:` line**, and the on-chain view says which protocol an `OP_RETURN` belongs to rather than calling all of them a "swap/LP memo".
...
* `--backend chainflip` — an independent cross-chain protocol, so it keeps working when THORChain and Maya halt together (as they did on 2026-08-18 — see [docs/halt-alternatives.md](docs/halt-alternatives.md)). It settles as a *vault swap*: the swap's parameters travel in your own transaction, with no broker, no deposit channel and nothing registered on your behalf. That takes two shapes. **From BTC**, one ordinary Bitcoin transaction paying a protocol vault, with the destination and a minimum-output floor in its OP_RETURN. **From ETH, ARB or their USDC/USDT**, a call into the protocol's Vault contract (plus an `approve` for a token), with the destination and a minimum-*price* floor in its arguments. Either way the gate decodes those bytes **itself** before signing and checks they pay your address, refund to your address, clear your floor, and carry no broker/boost/affiliate fee; the vault must appear in the list the chain publishes. Destinations are the assets the gate can re-derive an address for — the EVM ones (ETH/ARB native and USDC/USDT) from any source, and **BTC from an EVM source**, whose address the protocol carries verbatim. Tron and Solana quote but don't execute there, and `auto` routes around them while *saying so* when they were the cheaper route. `--amount max` can't be a vault swap *from BTC* — Chainflip needs a change output above dust to refund to — but is fine from an EVM source. See [docs/chainflip-effort.md](docs/chainflip-effort.md) and [docs/chainflip-evm.md](docs/chainflip-evm.md).
...
`blockstream.info`, fall back to `mempool.space` — when the first
stalls *or* rate-limits the request; setting this uses that endpoint
alone, so a throttle there can only be waited out), Ethereum
...
- **CoW Protocol** — **done** (`--backend cow`/`auto`): same-chain ETH-token
  ...
  *intent* model (sign a structured EIP-712 order, solvers settle) that fits
  ...
  independently gated.
- **Chainflip** — **done for BTC and EVM sources** (`--backend chainflip`, and
  in `auto`): a second *independent* non-custodial cross-chain venue that
  price-competes on BTC/ETH and keeps working when THORChain and Maya halt
  together. Executes as a **vault swap**, which takes the shape of its source
  chain: one Bitcoin transaction paying a protocol vault with the parameters in
  an OP_RETURN, or a call into the Vault contract from ETH/ARB. Quoting still
  covers more pairs than executing — a Tron source prices but does not settle,
  and neither do Tron and Solana destinations. It also reaches **SOL and DOT**,
  which need wallet keys of their own first.
...
```sh
curl -s "https://gateway.liquify.com/chain/thorchain_api/thorchain/quote/swap?from_asset=BTC.BTC&to_asset=ETH.ETH&amount=178100"
curl -s "https://gateway.liquify.com/chain/thorchain_api/thorchain/inbound_addresses"
```
...
- [docs/chainflip-effort.md](docs/chainflip-effort.md) · [docs/chainflip-evm.md](docs/chainflip-evm.md) — Chainflip vault swaps: the Bitcoin shape, and the EVM contract call
- [docs/chainflip.md](docs/chainflip.md) — superseded Chainflip notes (deposit channels, broker decision), kept for the reasoning
