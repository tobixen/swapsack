...
def test_dust_amount_raises_live():
    ...
    # Which error we get is CoW's routing, not our contract, and it is not even
    # stable between runs minutes apart: the fee check and both liquidity
    # checks all fire down here (seen so far: SellAmountDoesNotCoverFee,
    # InsufficientLiquidity, NoLiquidity). Assert the shape we parse — an
    # errorType naming fee or liquidity, then a description — rather than which
    # of them the solvers happened to reach first.
    with pytest.raises(CowError, match=r"^\w*(Fee|Liquidity)\w*: \S"):
        with CowClient() as client:
